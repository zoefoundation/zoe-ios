/// MVP signing credentials for Story 2.5.
/// Uses the same test certificate chain from contentauth/c2pa-ios for now.
/// Replace with a real cert chain certified against the SE public key before production release.
enum ZoeSigningCredentials {
    static let certsPEM = """
    -----BEGIN CERTIFICATE-----
    
    LQUJEtipZ0TqvI/4mieoyRCeIiQtyuS0LACz
    -----END CERTIFICATE-----
    -----BEGIN CERTIFICATE-----
    
    -----END CERTIFICATE-----
    """
}
